...
fn horizontal_product(matrix : &Vec<Vec<u64>>, row : usize, mut column : usize, size : usize) -> u64 {
    let mut product = 1;
    for _ in 0..size {
...
fn forward_product(matrix : &Vec<Vec<u64>>, mut row : usize, mut column : usize, size : usize) -> u64 {
    let mut product = 1;
    for _ in 0..size {
        product *= matrix[row][column];
        column += 1;
...
fn vertical_product(matrix : &Vec<Vec<u64>>, mut row : usize, column : usize, size : usize) -> u64 {
    let mut product = 1;
    for _ in 0..size {
        product *= matrix[row][column];
        row += 1;
...
fn backward_product(matrix : &Vec<Vec<u64>>, mut row : usize, mut column : usize, size : usize) -> u64 {
    let mut product = 1;
    for _ in 0..size {
        product *= matrix[row][column];
        column -= 1;
...
pub fn execute(input: &String) {
    ...
    for row in 0..19 {
        for column in 0..19 {
            if column < (20 - size + 1) {
                greatest_product = cmp::max(greatest_product, horizontal_product(&matrix, row, column, size));
            }
            if column < (20 - size + 1) && row < (20 - size + 1) {
                greatest_product = cmp::max(greatest_product, forward_product(&matrix, row, column, size));
            }
            if row < (20 - size + 1) {
                greatest_product = cmp::max(greatest_product, vertical_product(&matrix, row, column, size));
            }
            if row < (20 - size + 1) && column > (size - 1) {
                greatest_product = cmp::max(greatest_product, backward_product(&matrix, row, column, size));
            }
        }
    }
